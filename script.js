        ...
        if(user){
            ...
            try{
                const { data: prof } = await supabaseClient
                    .from("profiles")
                    .select("avatar_emoji, total_topup")
                    .eq("user_id", user.id)
                    .maybeSingle();
                if(prof){
                    avatarEmoji = prof.avatar_emoji || "🙂";
                    frameClass = getAccountFrameClass(Number(prof.total_topup || 0));
                }
            }catch(e){
                console.error("[updateUser] profile fetch error:", e);
            }
            btn.innerHTML = frameClass
                ? '<span class="u-frame-wrap ' + frameClass + '" style="width:100%;height:100%;"><span class="u-avatar" style="width:100%;height:100%;border:none;background:transparent;font-size:20px;">' + avatarEmoji + '</span></span>'
                : avatarEmoji;
...
const LB_RANK_META = [
  { badge:"👑 ອັນດັບ 1", icon:"👑", showCrown:true },
  { badge:"🥈 ອັນດັບ 2", icon:"🥈", showCrown:false },
  { badge:"🥉 ອັນດັບ 3", icon:"🥉", showCrown:false },
  { badge:"🔥 ອັນດັບ 4", icon:"🔥", showCrown:false }
];
...
async function loadLeaderboard(){
  ...
  try{
    ...
    track.innerHTML = data.map(function(u, i){
      ...
      const avatar = escapeHtmlTicker(u.avatar_emoji || "🙂");
...
async function loadUserTicker(){
  ...
  try{
    ...
    const chipsHtml = data.map(function(u){
      const name = escapeHtmlTicker(u.display_name || "ຜູ້ໃຊ້");
      const avatar = escapeHtmlTicker(u.avatar_emoji || "🙂");
      const frameClass = getTickerFrameClass(Number(u.total_topup || 0));
      return (
        '<div class="user-chip">' +
