...
// ===============================
// ດຶງຮູບສິນຄ້າແທ້ຈາກຕາຕະລາງ product_images (ທີ່ແອດມິນອັບໂຫລດໄວ້)
// ===============================
...
function renderProducts(){
  ...
  grid.innerHTML = list.map(function(p, i){
    ...
    const imgTag = p.image
      ? '<img class="card-img" src="' + p.image + '" alt="' + shownName + '">'
      : "";
    ...
    return (
      '<div class="card" data-cat="' + p.cat + '">' +
        ...
        '<div class="card-body">' +
          ...
          '<button class="buy-btn" ' + disabledAttr + ' onclick="goProduct(\'' + p.name.replace(/'/g,"") + '\',' + p.price + ',' + p.stock + ',\'' + shownName.replace(/'/g,"") + '\')">🛒 ສັ່ງຊື້ຕອນນີ້ເລີຍ</button>' +
        '</div>' +
      '</div>'
    );
  }).join("");
...
async function goProduct(name, price, stock, displayName){
    const {data:{user}} = await supabaseClient.auth.getUser();
    if(!user){
        openLogin();
        return;
