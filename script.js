    ...
    return (
      '<div class="card" data-cat="' + p.cat + '">' +
        ...
        '<div class="card-body">' +
          ...
          '<button class="buy-btn" ' + disabledAttr + ' onclick="goProduct(\'' + p.name.replace(/'/g,"") + '\',' + p.price + ',' + p.stock + ',\'' + shownName.replace(/'/g,"") + '\',\'' + p.cat + '\')">🛒 ສັ່ງຊື້ຕອນນີ້ເລີຍ</button>' +
        '</div>' +
      '</div>'
    );
  }).join("");
...
async function loadStockFromSupabase(){
  ...
  PRODUCTS.forEach(function(p){
    ...
    Object.keys(counts).forEach(function(key){
      if(key !== p.name && key.indexOf(prefix) === 0){
        total += counts[key];
      }
    });
    p.stock = total;
...
async function goProduct(name, price, stock, displayName, cat){
    const {data:{user}} = await supabaseClient.auth.getUser();
    if(!user){
        openLogin();
        return;
