...
const PRODUCTS = [
  ...
  {name:"PUBG Mobile", cat:"mobile-battle", price:50, stock:50, icon:"🔫"},
...
async function updateUser(){
    ...
    await refreshWalletBadge(user);
}

// ===============================
// กระเป๋าเงิน (wallet) + บัตรเติมเงิน
// ===============================

async function refreshWalletBadge(user){
    ...
    const { data, error } = await supabaseClient
        .from("wallets")
        .select("balance_lak, balance_thb")
        .eq("user_id", user.id)
        .maybeSingle();

    const balance = (!error && data) ? data.balance_lak : 0;
    badge.innerText = "💰 " + Number(balance || 0).toLocaleString("de-DE") + " ກີບ";
    badge.style.display = "inline-block";
}

function openTopup(){
    document.getElementById("topupMsg").innerText = "";
...
async function loadTopupBalance(){
    const {data:{user}} = await supabaseClient.auth.getUser();
    const el = document.getElementById("topupCurrentBalance");
    if(!user || !el) return;
    ...
    el.innerText = Number((data && data.balance_lak) || 0).toLocaleString("de-DE") + " ກີບ";
}
...
async function redeemCard(){
    ...
    if(!code){
        msg.style.color = "#ff5a5a";
        msg.innerText = "ກະລຸນາໃສ່ລະຫັດບັດ";
        return;
    }

    btn.disabled = true;
    msg.style.color = "#999";
    msg.innerText = "ກຳລັງເຕີມເງິນ...";

    const { data, error } = await supabaseClient.rpc("redeem_topup_card", { p_code: code });
    ...
    msg.style.color = "#7dff6a";
    msg.innerText = "ເຕີມເງິນສຳເລັດ +" + Number(data.amount).toLocaleString("de-DE") + " " + data.currency;
    document.getElementById("cardCodeInput").value = "";
