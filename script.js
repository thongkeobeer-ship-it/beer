...
function getFilteredProducts(){
  ...
  // ເຊື່ອງສິນຄ້າທີ່ແອດມິນຕັ້ງ is_hidden ໄວ້ ບໍ່ໃຫ້ສະແດງຢູ່ໜ້າເວັບເລີຍ (ຄົນລະແບບກັບ "ປິດຊົ່ວຄາວ" ທີ່ຍັງເຫັນການ໌ດ ແຕ່ຊື້ບໍ່ໄດ້)
  list = list.filter(function(p){ return !p.isHidden; });
...
async function loadProductOverrides(){
  const { data, error } = await supabaseClient
    ...
    .select("product_name, display_name, price, description, is_closed, close_note, is_hidden");
...
function renderProducts(){
  ...
  grid.innerHTML = list.map(function(p, i){
    ...
    return (
      '<div class="card' + (isClosed ? ' card-closed' : '') + (inStock && !isClosed ? ' in-stock' : '') + '" data-cat="' + p.cat + '">' +
  ...
  const visibleTotal = PRODUCTS.filter(function(p){ return !p.isHidden; }).length;
  const itemCount = document.getElementById("itemCount");
  if(itemCount) itemCount.innerText = "ເລືອກເບິ່ງສິນຄ້າ - ສະແດງ " + list.length + " ຈາກ " + visibleTotal + " ລາຍການ";
...
const ACCOUNT_BTN_TIERS = [
  { key:"legend",  min:3000000, frameClass:"frame-legend" },
...
function buildMiniFrameFx(frameClass){
  if(frameClass === "frame-red"){
    const positions = [
      "top:-5px;left:0px;transform:rotate(-12deg);",
      "top:11px;right:-7px;transform:rotate(18deg);",
      "bottom:-4px;left:9px;transform:rotate(165deg);"
    ];
    return positions.map(function(p, i){
      return '<span class="u-fx u-fx-bolt" style="' + p + 'animation-delay:' + (i * 0.3) + 's;"></span>';
    }).join("");
  }
  if(frameClass === "frame-gray"){
    const dotPositions = ["top:6px;left:-5px;", "bottom:3px;right:-5px;", "top:-4px;right:9px;"];
    const dotsHtml = dotPositions.map(function(p, i){
      return '<span class="u-fx u-fx-dot" style="' + p + 'animation-delay:' + (i * 0.45) + 's;"></span>';
    }).join("");
    const sparklePositions = ["top:-4px;left:5px;", "bottom:-5px;left:-3px;"];
    const sparklesHtml = sparklePositions.map(function(p, i){
      const shape = i % 2 === 0 ? "u-fx-gem" : "u-fx-star";
      return '<span class="u-fx ' + shape + '" style="' + p + 'animation-delay:' + (i * 0.4) + 's;"></span>';
    }).join("");
    return dotsHtml + sparklesHtml;
  }
  if(frameClass === "frame-blue"){
    const positions = ["top:-4px;left:1px;", "bottom:-4px;right:-3px;", "top:13px;left:-7px;"];
    return positions.map(function(p, i){
      return '<span class="u-fx u-fx-star" style="' + p + 'animation-delay:' + (i * 0.35) + 's;"></span>';
    }).join("");
  }
  if(frameClass === "frame-royal"){
    const sparklePositions = ["top:1px;left:-8px;", "bottom:1px;right:-8px;", "bottom:-5px;left:7px;"];
    const sparklesHtml = sparklePositions.map(function(p, i){
      const shape = i % 2 === 0 ? "u-fx-star" : "u-fx-gem";
      return '<span class="u-fx ' + shape + '" style="' + p + 'animation-delay:' + (i * 0.3) + 's;"></span>';
    }).join("");
    return '<span class="u-fx-rays"></span>' + sparklesHtml + '<span class="u-fx u-fx-crown" style="top:-12px;left:50%;"></span>';
  }
  if(frameClass === "frame-legend"){
    // เวอร์ชันย่อของกรอบ Legendary: ใช้บล็อก u-fx เดิม (bolt แทนเปลวไฟ, star/gem แทนนีออน) ให้เข้าธีมทันทีที่ index.html ผูกสไตล์ .frame-legend ไว้แล้ว
    const flamePositions = ["top:-5px;left:0px;transform:rotate(-12deg);", "top:11px;right:-7px;transform:rotate(18deg);", "bottom:-4px;left:9px;transform:rotate(165deg);"];
    const flamesHtml = flamePositions.map(function(p, i){
      return '<span class="u-fx u-fx-bolt" style="' + p + 'animation-delay:' + (i * 0.3) + 's;"></span>';
    }).join("");
    const sparklePositions = ["top:1px;left:-8px;", "bottom:1px;right:-8px;", "bottom:-5px;left:7px;"];
    const sparklesHtml = sparklePositions.map(function(p, i){
    ...
    return flamesHtml + sparklesHtml + '<span class="u-fx-rays"></span>';
  }
  return "";
}
...
async function updateUser(){
    ...
    if(btn){
        if(user){
            ...
            btn.classList.remove("acct-glow", "acct-glow-white", "acct-glow-blue", "acct-glow-red", "acct-glow-gray", "acct-glow-royal", "acct-glow-legend");
            const ACCT_GLOW_MAP = { "frame-white":"acct-glow-white", "frame-blue":"acct-glow-blue", "frame-red":"acct-glow-red", "frame-gray":"acct-glow-gray", "frame-royal":"acct-glow-royal", "frame-legend":"acct-glow-legend" };
...
const LB_RANK_META = [
  { badge:"👑 ອັນດັບ 1", icon:"👑", showCrown:true,  sparkles:["shape-star","shape-gem","shape-star","shape-gem"] },
  { badge:"🥈 ອັນດັບ 2", icon:"🥈", showCrown:false, sparkles:["shape-star","shape-gem","shape-star","shape-gem"] },
  { badge:"🥉 ອັນດັບ 3", icon:"🥉", showCrown:false, sparkles:["shape-star","shape-gem","shape-star","shape-gem"] },
  { badge:"🔥 ອັນດັບ 4", icon:"🔥", showCrown:false, sparkles:["shape-gem","shape-star","shape-gem","shape-star"] }
];
  ...
  try{
    ...
    track.innerHTML = data.map(function(u, i){
      ...
      const sparklesHtml = LB_SPARKLE_POSITIONS.map(function(p, si){
        return '<span class="lb-sparkle ' + meta.sparkles[si] + '" style="' + p + 'animation-delay:' + (si * 0.4) + 's;"></span>';
      }).join("");
...
const TICKER_TIERS = [
  { min:3000000, frameClass:"frame-legend" },
